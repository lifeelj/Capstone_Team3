...
처음에는 두더지가 랜덤한 위치에서 나타나도록 구현하였다. 그리고 이후에 나타나는 두더지들은 전부 나타났을 때 두더지 사이의 거리가 lognormal 분포를 따르도록 구현하였다. <br>
다음 두더지까지의 거리가 정해진 후에 어느 각도에 나타날 것인지도 정해줘야 했기 때문에 theta라는 변수를 두었고 이는 360도 내에서 무작위로 결정되도록 구현하였다. <br>
r과 theta값을 바탕으로 한 극좌표계를 카르테시안 좌표계로 변경해야 했기 때문에 x 변수와 y 변수를 두어서 x = prevx + rcos(theta), y = prevy + rsin(theta)로 계산하여 초기화하였다. 이때 x값과 y값이 두더지가 나타나는 범위를 벗어나게 된다면 새로운 좌표를 생성하도록 구현하였다.<br>
앞에서 계산한 x값과 y값을 margin left와 margin right를 사용해서 두더지에 적용하였다. 두더지는 x값만큼 왼쪽에서 멀어지게 되고, y값만큼 위쪽에서 멀어지게 되도록 구현하였다. 웹의 특성상 반응형으로 제작하게 되면 디바이스별로 두더지가 나타나는 위치가 달라질 수 있기 때문에 이러한 부분에서 모든 레이아웃에 정확한 px값을 설정하였다.<br>
